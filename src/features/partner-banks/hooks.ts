...
export function usePartnerBanks() {
  ...
  const updatePartnerBank = useCallback(async (id: string, partnerBankData: UpdatePartnerBankDto) => {
    try {
      setPartnerBanksState(prev => ({ ...prev, loading: true, error: null }));
      
      const updatedPartnerBank = await partnerBankService.updatePartnerBank(id, partnerBankData);
      
      setPartnerBanksState(prev => ({
        ...prev,
        partnerBanks: prev.partnerBanks.map(partnerBank => 
          partnerBank.id === id ? updatedPartnerBank : partnerBank
        ),
        loading: false,
      }));
  ...
  const deletePartnerBank = useCallback(async (id: string) => {
    try {
      setPartnerBanksState(prev => ({ ...prev, loading: true, error: null }));
      
      await partnerBankService.deletePartnerBank(id);
      
      setPartnerBanksState(prev => ({
        ...prev,
        partnerBanks: prev.partnerBanks.filter(partnerBank => partnerBank.id !== id),
        total: prev.total - 1,
        loading: false,
      }));
    } catch (error) {

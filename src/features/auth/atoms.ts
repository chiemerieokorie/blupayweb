...
export const partnerBankAtom = atomWithStorage<string | null>('partner-bank', null);
...
export const loginAtom = atom(
  null,
  async (get, set, credentials: { email: string; password: string; partnerBank?: string }) => {
    try {
      ...
      if (credentials.partnerBank) {
        response = await authService.loginWithPartnerBank(
          { email: credentials.email, password: credentials.password },
          credentials.partnerBank
        );
        set(partnerBankAtom, credentials.partnerBank);
      } else {
      ...
      apiClient.setAuth(response.token, credentials.partnerBank);
...
export const initializeAuthAtom = atom(
  ...
  async (get, set) => {
    try {
      ...
      const partnerBank = get(partnerBankAtom);
      
      if (token) {
        const { apiClient } = await import('@/sdk/client');
        apiClient.setAuth(token, partnerBank || undefined);
